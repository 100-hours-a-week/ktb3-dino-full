...
public class PostRepository {
    ...
    private static class PostEntity {
        ...
        PostEntity(Long id, String title, String author, String content, LocalDateTime createdAt){
            this.id=id; this.title=title; this.author=author; this.content=content; this.createdAt=createdAt;
        }
    ...
    public PostRepository(){
        save("Welcome!", "admin", "I love seoul Korea. Kimchi delicious");
    }
    ...
    public PostDto save(String title, String author, String content){
        Long id = postSeq.incrementAndGet();
        var e = new PostEntity(id, title, author, content, LocalDateTime.now());
        store.put(id, e);
        return e.toDto();
    }
    ...
    public EditStatus edit(Long postId, String newAuthor, String newTitle, String content) {
        var e = store.get(postId);
        if (e == null) return EditStatus.NOT_FOUND;
        return e.edit(newAuthor, newTitle, content);
    }

    public DeleteStatus delete(Long postId, String newAuthor) {
        var e = store.get(postId);
        if (e == null) return DeleteStatus.NOT_FOUND;
        if (!e.author.equals(newAuthor)) return DeleteStatus.FORBIDDEN;
        store.remove(postId);
